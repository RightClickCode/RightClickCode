...
describe("Spellcheck Executor", () => {
  ...
  it("empty options", async () => {
    ...
    expectCommandToHaveBeenCalled('cspell lint "**" --color');
  ...
  it("config", async () => {
    ...
    const output = await executor(options, mockContext);
    expectCommandToHaveBeenCalled('cspell lint "**" -c cspell.config.js --color');
    expect(output.success).toBe(true);
  });
  ...
  it("disable color", async () => {
    ...
    const output = await executor(options, mockContext);
    expectCommandToHaveBeenCalled('cspell lint "**"');
    expect(output.success).toBe(true);
  });
  ...
  it("config and disable color", async () => {
    ...
    const output = await executor(options, mockContext);
    expectCommandToHaveBeenCalled('cspell lint "**" -c cspell.config.js');
    expect(output.success).toBe(true);
  });
